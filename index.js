...
$("#js-moving-weather-link, #js-weather-link").click(function () {
    $('html, body').animate({
        scrollTop: $("#js-weather-results").offset().top - 150
    }, 3000);
    $("#js-weather-header").show()
    setTimeout(function () {
        $('#js-moving-links').show()
    }, 4000)

});

$("#js-moving-news-link, #js-news-link").click(function () {
    $('html, body').animate({
...
function getWeather(cityState, unitType) {
    ...
    fetch(url)
        ...
        .catch(err => {
            $('#js-hide').hide();
            $('#js-error-message-weather').empty().text(`Something went wrong; updating your input might help. `).show();
        })
};
